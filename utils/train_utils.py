...
from models.Nets import CNNCifar, CNNCifar100, CNNCifarRoD, CNNCifar100RoD, CNNCifarCP, CNNCifar100CP, RNNSent, MLP
...
def get_model(args):
    if args.model == 'cnn' and 'cifar100' in args.dataset and args.alg == 'fedcp':
        net_glob = CNNCifar100CP(args=args).to(args.device)
    elif args.model == 'cnn' and 'cifar10' in args.dataset and args.alg == 'fedcp':
        net_glob = CNNCifarCP(args=args).to(args.device)
    elif args.model == 'cnn' and 'cifar100' in args.dataset and args.alg != 'fedrod':
        net_glob = CNNCifar100(args=args).to(args.device)
    elif args.model == 'cnn' and 'cifar10' in args.dataset and args.alg != 'fedrod':
        net_glob = CNNCifar(args=args).to(args.device)
    elif args.model == 'mlp' and 'mnist' in args.dataset and args.alg != 'fedrod':
        net_glob = MLP(dim_in=784, dim_hidden=256, dim_out=args.num_classes).to(args.device)
    elif args.model == 'cnn' and 'femnist' in args.dataset and args.alg != 'fedrod':
        net_glob = CNN_FEMNIST(args=args).to(args.device)
    elif args.model == 'mlp' and 'cifar' in args.dataset and args.alg != 'fedrod':
        net_glob = MLP(dim_in=3072, dim_hidden=512, dim_out=args.num_classes).to(args.device)
    elif 'sent140' in args.dataset and args.alg != 'fedrod':
        net_glob = model = RNNSent(args,'LSTM', 2, 25, 128, 1, 0.5, tie_weights=False).to(args.device)
    elif args.model == 'cnn' and 'cifar100' in args.dataset and args.alg == 'fedrod':
        net_glob = CNNCifar100RoD(args=args).to(args.device)
    elif args.model == 'cnn' and 'cifar10' in args.dataset and args.alg == 'fedrod':
        net_glob = CNNCifarRoD(args=args).to(args.device)
