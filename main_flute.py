...
from models.Update import LocalUpdate, ServerUpdateNC2, fun_reg, fun_reg_2
...
if __name__ == '__main__':
    ...
    if args.alg == 'fedrep' or args.alg == 'fedper' or args.alg == 'flute':
        if 'resnet' in args.model:
            if 'cifar' in args.dataset:
                w_glob_keys = [net_glob.weight_keys[i] for i in range(22)]
        else:
            if 'cifar' in args.dataset:
    ...
    # print(w_glob_keys)
    # print(net_keys)
    if args.alg in ['fedrep', 'fedper', 'lg', 'flute']:
        num_param_glob = 0
        num_param_local = 0
    ...
    with open("/home/anaconda/content/flute/save/regularization.txt", 'w') as f:
        pass
    with open("/home/anaconda/content/flute/save/regularization_g.txt", 'w') as f:
        pass
    with open('/home/anaconda/content/flute/save/NC2_' + args.alg + '_' + str(
            args.local_ep) + 'eps' + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
            args.shard_per_user) + '.txt', "w") as f:
        pass
    with open('/home/anaconda/content/flute/save/Local_NC2_' + args.alg + '_' + str(
            args.local_ep) + 'eps' + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
            args.shard_per_user) + '.txt', "w") as f:
        pass
    ...
    start = time.time()
    w_glob_copy = {}
    for iter in trange(args.epochs + 1, desc = "Training:"):
        w_glob = {}
        loss_locals = []
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                if args.epochs == iter:
                    local = LocalUpdate(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_ft]],
                                        idxs=dict_users_train, indd=idx)
                else:
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                ...
                w_local, loss, indd = local.train(net=net_local.to(args.device), ind=idx, w_glob_keys=w_glob_keys,
        ...
        if iter % args.save_every == args.save_every - 1:
            model_save_path = '/home/anaconda/content/flute/save/accs_' + args.alg + '_' + args.dataset + '_' + str(
                args.num_users) + '_' + str(args.shard_per_user) + '_iter' + str(iter) + '.pt'
            torch.save(net_glob.state_dict(), model_save_path)


        if args.alg == 'flute' and args.server_update == 1:
            server_update = ServerUpdateNC2(args, w_locals, idxs_users)
            w_locals = server_update.train()

        loss_g, lloss_g = fun_reg(args, w_locals)
        # loss_g = fun_reg_2(args, w_locals, idxs_users)
        with open('/home/anaconda/content/flute/save/NC2_' + args.alg + '_' + str(
                args.local_ep) + 'eps' + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
                args.shard_per_user) + '.txt', "a") as f:
            f.write(str(loss_g) + "\n")
        with open('/home/anaconda/content/flute/save/Local_NC2_' + args.alg + '_' + str(
                args.local_ep) + 'eps' + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
                args.shard_per_user) + '.txt', "a") as f:
    ...
    print(times)
    print(accs)
    base_dir = '/home/anaconda/content/flute/save/accs_' + args.alg + '_' + str(
        args.local_ep) + 'eps' + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
    ...
    if args.alg == 'fedavg':
        global_accs_dir = '/home/anaconda/content/flute/save/global_accs_' + args.alg + '_' + str(
            args.local_ep) + 'eps' + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
            args.shard_per_user) + '.csv'
        accs_global = np.array(accs_global)
        accs_global = pd.DataFrame(accs_global, columns=['accs'])
        accs_global.to_csv(global_accs_dir, index=False)
